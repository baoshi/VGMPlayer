/* fft-m0.c
 *
 * Based on Adafruit_ZeroFFT
 *
 * Original Author: Dean Miller
 */
#include "fpsqrt.h"
...
// Work buffer
static q15_t ALIGN4 scratchData[FFT_WORKAREA];
...
void arm_radix2_butterfly_q15(q15_t* pSrc, uint32_t fftLen, q15_t* pCoef, uint16_t twidCoefModifier) 
{
    ...
    for (j = 0; j < n2; j++) 
    {
        ...
        for (i = j; i < (int)fftLen; i += n1) 
        {
            ...
            pSrc[2 * i + 1] = ((pSrc[2 * l + 1] >> 2u) + (pSrc[2 * i + 1] >> 2u)) >> 1u;
...
int fft_q15(q15_t *source, uint16_t length)
{
    ...
    for (int i = 0; i < length; i++) 
    {
        int32_t real = (int32_t)(*pOut++);  // q15
        int32_t imaginary = (int32_t)(*pOut++); // q15
        int32_t re = real * real; // q30
